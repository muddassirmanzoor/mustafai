...
@push('header-scripts')
  ...
  <style>
    div#occupations-datatable_length {
    padding-top: 24px;
    }
    div#occupations-datatable_filter {
    margin-top: -41px;
    }
</style>
@endpush
				    ...
				    <div class="card">
				      <div class="card-header">
				      	@if(have_right('Create-Occupations'))
					        <h3 class="card-title">
					        	<a href="{{ URL('admin/occupations/create') }}" class="btn btn-primary"> Add New </a>
					        </h3>
				        @endif
				      </div>
				      <div class="card-body">
				        <table id="occupations-datatable" class="table table-bordered table-striped" style="width:100%">
				          <thead>
				              <tr>
				                <th>ID</th>
				                <th>Slug</th>
...
@push('footer-scripts')
  <!-- jQuery -->
	...
	<script>
	  $(function () {
	    $('#occupations-datatable').dataTable(
      	{
      		sort: false,
            buttons: dataCustomizetbaleAdmin("Professions",  arrywidth= [ '33%', '33%','33%'],  arraycolumn =[0,1,2]),
			pageLength: 50,
			scrollX: true,
			processing: false,
