				    ...
				    <div class="card">
				      <div class="card-header">
				      	@if(have_right('Create-Pages'))
					        <h3 class="card-title">
					        	<a href="{{ URL('admin/pages/create') }}" class="btn btn-primary"> Add New </a>
					        </h3>
				        @endif
				      </div>
				      <div class="card-body">
...
@push('footer-scripts')
  <!-- jQuery -->
	    ...
	    $('#pages-datatable').dataTable(
      	{
			...
			responsive: true,
			dom: 'Blfrtip',
            buttons: dataCustomizetbaleAdmin("Pages",  arrywidth= [ '16%', '22%', '40%','22%'],  arraycolumn = [0,1,2,4]),
	...
	function is_featured(_this,id){
		var URL = "{{ URL('admin/pages/featured-product') }}";
		...
		$.ajax({
			...
			if(response == '1'){
				...
				swal("Error!", "You can not set more than 5 pages as featured, please remove any one", "error");
			}
			},
			error: function(data) {
			}
