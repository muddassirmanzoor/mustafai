	...
	<div class="content-wrapper">
		...
		<div class="content-header">
		  <div class="container-fluid">
		    <div class="row mb-2">
		      <div class="col-sm-6">
		        <h1 class="m-0">{{ ($action=='add') ? 'Create' : 'Edit' }} Slider </h1>
		      </div><!-- /.col -->
		      <div class="col-sm-6">
		        <ol class="breadcrumb float-sm-right">
		          <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Home</a></li>
		          <li class="breadcrumb-item"><a href="{{ URL('admin/slider') }}"> Slider</a></li>
		          <li class="breadcrumb-item active">{{ ($action=='add') ? 'Create' : 'Edit' }} Slider</li>
		        </ol>
		      </div><!-- /.col -->
		    </div><!-- /.row -->
		  </div><!-- /.container-fluid -->
			                ...
			                <form id="page-form" class="form-horizontal label-left" action="{{ URL('admin/slider') }}" enctype="multipart/form-data" method="POST">
								...
								<div class="accordion" id="accordionExample">
									<div class="card">
										...
										<div id="title" class="collapse" aria-labelledby="title-heading" data-parent="#accordionExample">
											<div class="card-body">
												<div class="form-group row">
													<label class="col-sm-2 col-form-label">Message Title <span class="text-red">*</span></label>
													<div class="col-sm-6">
										...
										<div id="image" class="collapse" aria-labelledby="message-title-heading" data-parent="#accordionExample">
											<div class="card-body">
												<div class="form-group row">
													...
													<div class="col-sm-6">
														<input type="file"  class="form-control" placeholder="Select Image" id="imageinpt" name="image" value="" @if(!$row->image) required @endif>
														<small class="d-block text-muted  mt-1">Please add image file as per mentioned restrictions <span class="text-danger"> 2560 x 950 </span> pixels</small>
													</div>
													<div class="col-sm-2">
														@if($row->image)
                                        ...
                                        <!-- For Mobile App Slider Content   -->
										<div class="card-header" id="title-heading">
											<h2 class="mb-0">
												<button class="btn btn-link" type="button" data-toggle="collapse" data-target="#app-content" aria-expanded="true" aria-controls="title">
                                                    Mobile App Slider Content
												</button>
											</h2>
										</div>

										<div id="app-content" class="collapse" aria-labelledby="title-heading" data-parent="#accordionExample">
											<div class="card-body">
                                                @foreach (activeLangs() as $lang)
                                                    <div class="form-group row">
                                                        <label class="col-sm-2 col-form-label">Button Text ({{ $lang->name_english }})</label>
                                                        <div class="col-sm-6">
                                                            <input type="text" class="form-control" placeholder="Enter Button Text in {{ $lang->name_english }}" name="btn_text_{{ $lang->key }}" value="{{$row->{'btn_text_'.$lang->key} }}" >
                                                        </div>
                                                    </div>
                                                @endforeach
												<div class="form-group row">
													...
													<div class="col-sm-6">
														<input type="text" class="form-control" placeholder="Enter URL " name="btn_link" value="{{ $row->btn_link }}">
													</div>
												</div>
                                                <div class="row">
                                                    @foreach (activeLangs() as $lang)
													<div class="col-sm-4">
														<div class="form-group ">
															<label class="col-form-label">Mobile App Slider Content ({{ $lang->name_english }})</label>
															<textarea  class="form-control" placeholder="Enter Mobile App Content in {{ $lang->name_english }}" rows="5" name="mobile_content_{{ $lang->key }}" >{{$row->{'mobile_content_'.$lang->key} }}</textarea>
														</div>
													</div>
                                                    @endforeach
